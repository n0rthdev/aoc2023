...
class Day03Part2 : LinesPuzzle() {
    ...
    override fun solve(lines: List<String>): String {
        ...
        val gearSymbols = symbols.filter {
            it.value.symbol == '*'
        }

        val gearRatios = gearSymbols.mapNotNull { gear ->
            val adjacentNumbers = numbers.filter { it.value.allPos.any { it.adjecentTo(gear.value.pos) } }
            if (adjacentNumbers.size == 2) {
                adjacentNumbers.values.first().value * adjacentNumbers.values.last().value
            } else {
                null
            }
        }
