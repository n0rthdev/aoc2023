...
//                }
    ...
    companion object {
        data class Instruction(val dirCode: Char, val count: Int, val colorCode: String) {
            val dir = parser.parse(dirCode)
            val totalMove get() = dir * count
            val allSteps get() = (1..count).map { dir }

            companion object {
                val parser = Vector3D.Companion.DirectionParser.fromChars("RLUDXX")
                fun parse(instruction: String): Instruction {
                    val parts = instruction.split(" ")
                    return Instruction(parts[0].first(), parts[1].toInt(), parts[2].trim('(', ')'))
                }
            }
        }
    }
